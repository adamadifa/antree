...
@section('header', 'Register New Counter')
...
<div class="max-w-3xl space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            <h1 class="text-xl font-bold text-slate-800 tracking-tight">Create Counter</h1>
    ...
    <div class="bg-white rounded-3xl border border-slate-200/85 shadow-sm overflow-hidden flex flex-col">
        ...
        <div class="px-8 py-5 bg-slate-50/50 border-b border-slate-100 flex items-center justify-between">
            <div>
                <h3 class="text-sm font-bold text-slate-800">Counter Registration</h3>
                <p class="text-[11px] text-slate-400 font-medium">Assign a new service point and operator.</p>
            </div>
            <div class="w-8 h-8 bg-rose-50 rounded-lg flex items-center justify-center">
                <svg class="w-4 h-4 text-rose-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
            </div>
        </div>

        <div class="p-8">
            <form id="service-form" action="{{ route('admin.counters.store') }}" method="POST" class="space-y-6" novalidate>
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-7">
                    ...
                    <div class="space-y-1 md:col-span-2">
                        <div class="relative">
                            ...
                            <div id="wrapper-name" class="flex items-center border rounded-2xl px-4 py-1.5 bg-white transition duration-200 focus-within:ring-4 @error('name') border-red-300 ring-red-50 focus-within:border-red-400 focus-within:ring-red-50 @else border-slate-200 focus-within:border-rose-450 focus-within:ring-rose-50 @enderror">
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="e.g. Loket 1, Counter A" 
                    ...
                    <div class="space-y-1">
                        <div class="relative">
                            ...
                            <div id="wrapper-number" class="flex items-center border rounded-2xl px-4 py-1.5 bg-white transition duration-200 focus-within:ring-4 @error('number') border-red-300 ring-red-50 focus-within:border-red-400 focus-within:ring-red-50 @else border-slate-200 focus-within:border-rose-450 focus-within:ring-rose-50 @enderror">
                                ...
                                <input type="number" name="number" id="number" value="{{ old('number') }}" placeholder="e.g. 1, 2, 3" 
                    ...
                    <div class="space-y-1">
                        <div class="relative">
                            ...
                            <div id="wrapper-user_id" class="flex items-center border rounded-2xl px-4 py-1.5 bg-white transition duration-200 focus-within:ring-4 @error('user_id') border-red-300 ring-red-50 focus-within:border-red-400 focus-within:ring-red-50 @else border-slate-200 focus-within:border-rose-450 focus-within:ring-rose-50 @enderror">
                                <svg class="w-5 h-5 text-slate-400 shrink-0 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <select name="user_id" id="user_id" onchange="validateSelect('user_id', this.value)"
                                    ...
                                    @foreach($operators as $operator)
                                        <option value="{{ $operator->id }}" {{ old('user_id') == $operator->id ? 'selected' : '' }}>{{ $operator->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
<script>
    ...
    function validateNumber(value) {
        ...
        numberTimeout = setTimeout(() => {
            fetch('{{ route('admin.counters.check-number') }}?number=' + encodeURIComponent(value))
