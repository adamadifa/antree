...
@section('sky_layout')
    ...
    <div class="relative min-h-screen w-full flex items-center justify-center p-4 overflow-hidden select-none" style="background: linear-gradient(180deg, rgba({{ $primaryRgb }}, 0.22) 0%, rgba({{ $primaryRgb }}, 0.08) 55%, #ffffff 100%);">
        ...
        <div class="absolute top-8 left-8 flex items-center gap-2.5 z-20">
            @if(!empty($logoUrl))
                <img src="{{ $logoUrl }}" class="w-8 h-8 object-contain rounded-xl shadow-sm" alt="Logo">
            @else
            ...
            @endif
            <span class="font-bold text-slate-800 text-lg tracking-tight">{{ $appName }}</span>
        </div>

        <!-- Glassmorphic Login Card -->
        <div class="w-full max-w-[440px] bg-white/40 backdrop-blur-xl border border-white/50 rounded-[2.5rem] p-8 md:p-10 relative z-10 flex flex-col items-center" style="box-shadow: 0 25px 50px -12px rgba({{ $primaryRgb }}, 0.15);">
            ...
            <form method="POST" action="{{ route('login') }}" class="w-full">
                ...
                <!-- Email Address -->
                <div class="mb-4">
                    <div class="relative flex items-center bg-[#f1f5f9]/70 focus-within:bg-white rounded-2xl transition duration-150 ease-in-out px-4 py-3.5 border border-transparent focus-within:border-slate-200/50 focus-primary">
                ...
                <!-- Login Button -->
                <button type="submit" class="w-full text-white font-semibold py-3.5 px-4 rounded-2xl shadow-sm text-sm mb-6 btn-primary-custom">
                    Sign In
                </button>
